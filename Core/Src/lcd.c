...
void LCD_DrawName(uint16_t arr[], uint16_t length, uint16_t width) {
	...
	for (uint16_t i = 0; i < length; i++) {
		uint16_t color = arr[i] ? WHITE : BLACK;
		uint16_t column = i % width;
		uint16_t page = i / width;
		LCD_DrawDot(column + xOff, page + yOff, color);
	}
}
...
{	
	...
	for (float x = usC - LR; x <= usC + LR; x += 1){
		if (x >= 0 && x <= 241) {
			float yPos =
					SR * sqrt(1 - pow(((x - usC) / LR), 2)) + usP;
			float yNeg =
					-SR * sqrt(1 - pow(((x - usC) / LR), 2)) + usP;

			if (yPos >= 0 && yPos <= 321) {
				LCD_DrawDot(round(x), round(yPos), usColor);
			}
			if (yNeg >= 0 && yNeg <= 321) {
				LCD_DrawDot(round(x), round(yNeg), usColor);
			}
		}
	}

	for (float y = usP - SR; y <= usP + SR; y += 1){
		if (y >= 0 && y <= 321) {
			float xPos =
					LR * sqrt(1 - pow(((y - usP) / SR), 2)) + usC;
			float xNeg =
					-LR * sqrt(1 - pow(((y - usP) / SR), 2)) + usC;

			if (xPos >= 0 && xPos <= 241) {
				LCD_DrawDot(round(xPos), round(y), usColor);
			}
			if (xNeg >= 0 && xNeg <= 241) {
				LCD_DrawDot(round(xNeg), round(y), usColor);
			}
		}
	}
}
